...
EXPORT void *create_std_function_wrapper(void (*c_callback)(const QVariant *, void *), void *user_data)
{
    if (!c_callback)
        return nullptr;

    // Create a std::function that captures the C callback and user_data
    auto wrapper = new StdFunctionWrapper([c_callback, user_data](const QVariant &result) {
